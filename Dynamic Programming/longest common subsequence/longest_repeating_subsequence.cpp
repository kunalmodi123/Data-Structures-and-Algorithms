...
int lcsTab(string x, string y, int m, int n)
{
    ...
    for (int i = 1; i < m + 1; i++)
    {
        for (int j = 1; j < n + 1; j++)
        {
            if (x[i - 1] == y[j - 1] && i != j) // important condition
                t[i][j] = 1 + t[i - 1][j - 1];
            else
...
int main()
{
    ...
    int count = lcsTab(x, y, m, n); // lcs of same string with some modifications(i.e. i != j)

    cout << "longest repeating subsequence length: " << count << "\n";
